    ...
    def build(self):
        ...
        class BinomialLogisticRegressionModel(torch.nn.Module):
            ...
            def forward(self, x, a):
                # Compute linear transformation x.A.T
                return torch.sigmoid(self.pairwise_dot(x, self.weight[a,:]) + self.bias) 
        ...
        # Convert data to torch objects - non-clicks for learning multinomial model
        X0 = Variable(torch.Tensor(X[y == 0]))
        A0 = Variable(torch.LongTensor(A[y == 0]))
        w0 = torch.Tensor(pss[y == 0] ** -1)
        N0 = X0.shape[0]
        ...
        def closure():
            ...
            if self.config.alpha < 1.0:
                ...
                p_a = torch.gather(p_a, 1, A1.unsqueeze(1)).reshape(-1)
            ...
            if .0 < self.config.alpha:
                ...
                # Negative log-likelihood as loss here
...
# EVERYTHING BELOW THIS LINE WAS GRACIOUSLY TAKEN FROM 
